  ...
  if (fetchJson) {
    ...
    for (pluginId in fetchJson) {
      ...
      if (hasVariables) {
        ...
        // logging.logger.info('features', features)
        ...
        variableList.forEach(function(variable) {
          // logging.logger.info('Looking up variable:', variable)
...
State.clearState = function clearState(appDirectory, options) {
  if (!options) { 
    options = {};
    options.platforms = options.plugins = true;
  }

  if (!options.platforms && !options.plugins) {
    logging.logger.error('You must specify either platforms or plugins to clear the state of your Ionic application.');
    return;
  }
  ...
  if (options.platforms) {
    logging.logger.info('Clearing Platforms\n'.blue.bold)
    shelljs.rm('-rf', platformPath);
    packageJson.cordovaPlatforms = [];
  }

  if (options.plugins) {
    logging.logger.info('Clearing Plugins\n'.blue.bold);
