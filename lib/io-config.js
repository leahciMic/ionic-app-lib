...
IoConfig.isCoreAvailable = function isCoreAvailable(appDirectory) {
  if (!appDirectory) {
    return Q.reject('You must specify the app directory to check core packages');
  }

  var deferred = Q.defer();
  var coreFilePath = path.join(appDirectory, CORE_FILE);
  fs.exists(coreFilePath, function(exists) {
    if (exists) {
...
IoConfig.doesHaveKey = function doesHaveKey(key, appDirectory) {
  if (!appDirectory) {
    return Q.reject('You must specify the app directory of the key you wish to check');
  }

  try {
    var configJson = IoConfig.readConfigFile(appDirectory);
    return Q.resolve(configJson && configJson[key]);
  } catch (ex) {
    return Q.reject(ex);
  }
};

IoConfig.listConfig = function listConfig(appDirectory) {
  if (!appDirectory) {
    throw 'You must specify the app directory of the config you wish to list.';
  }
  var configPath = path.join(appDirectory, CONFIG_BACKUP);
...
IoConfig.warnMissingData = function warnMissingData(appDirectory) {
  if (!appDirectory) {
    throw new Error('You must specify the app directory of the config you wish to list.');
  }
...
IoConfig.writeIoConfig = function writeIoConfig(key, val, set, appDirectory) {
  if (!appDirectory) {
    throw 'You must specify the app directory of the io-config you wish to write to.';
  }
  ...
  if (ALLOWED_CONFIG[key] || key == false) {
    ...
    fs.readFile(configPath, function(err, data) {
      ...
      fs.writeFile(configPath, JSON.stringify(jsonObj), function(error) {
        if (error) {
...
IoConfig.injectIoComponent = function injectIoComponent(set, component, appDirectory) {
  if (!appDirectory) {
    throw 'You must specify the app directory of where you want the components injected';
  }
  ...
  if (IO_COMPONENTS[component]) {
    ...
    fs.readFile(configPath, function(e, d) {
      var disable = false;
      if (d) {
...
IoConfig.initIoPlatform = function initIoPlatform(appDirectory, jar) {
  ...
  try {
    return IoConfig.dashInit(project, jar).then(function(key){
      ...
      IoConfig.writeIoConfig('app_id', key.app_id, true, appDirectory).then(function(obj){
        if (key.api_key) {
          IoConfig.writeIoConfig('api_key', key.api_key, true, appDirectory).then(function(apiKey){
...
IoConfig.readConfigFile = function readConfigFile(appDirectory) {
  if (!appDirectory) {
    throw new Error('You must specify the app directory to read the config file for.');
  }
