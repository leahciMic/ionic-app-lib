...
Start.startApp = function startApp(options) {
  ...
  ioLib.warnMissingData(options.targetPath);
  ...
  .catch(function(err) {
    events.emit('log', 'Error Initializing app: ', err);
...
Start.fetchZipStarter = function fetchZipStarter(options) {
  ...
  console.log('Fetching ZIP from url:', options.zipFileDownload.bold, 'to: ', options.targetPath);
...
Start.initCordovaFromGui = function initCordovaFromGui(options, appSetup) {
  ...
  events.emit('verbose', 'Initializing Cordova for Gui');
