...
Upload.doUpload = function doUpload(appDirectory, jar, note, deploy) {
  ...
  try {
    ...
    .then(function(key){
      ...
      ioLib.writeIoConfig('app_id', key.app_id, true, appDirectory)
      .then(function(key){
        // Do nothing
      }, function(error) {
    ...
    .then(function(status) {
      ...
      if (status && (typeof status === 'object') && status.version) {
        ...
        if (status.api_key) {
          ioLib.writeIoConfig('api_key', status.api_key, true, appDirectory).then(function(apiKey){
            ioLib.warnMissingData(appDirectory);
          }, function(error) {
